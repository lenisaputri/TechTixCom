...
$pdf->Cell(26.8,0.5,"HASIL NILAI TRAINING TECHNICAL ONLINE",0,1,'C');
$pdf->ln(1);
...
$pdf->Cell(2, 0.8, 'POIN', 1, 0, 'C');
$pdf->Cell(2, 0.8, 'NILAI', 1, 0, 'C');
...
$from=$_POST['from'];
$end=$_POST['end'];
$query=mysqli_query($con,"SELECT tst.*, tp.id_operator, tp.nik, tp.nama as nama_lengkap, tj.nama as nama_jabatan,tstd.* 
FROM tabel_score_technical tst, tabel_operator tp , tabel_score_technical_detail tstd, tabel_jabatan tj 
WHERE tst.id_operator = tp.id_operator 
AND tp.id_jabatan = tj.id_jabatan 
AND tst.id_score_technical = tstd.id_score_technical
AND (tst.tanggal_training BETWEEN '$from' AND '$end')");
while($lihat=mysqli_fetch_array($query)){
	...
	$pdf->Cell(2, 0.8, $lihat['poin'], 1, 0,'C');
	$pdf->Cell(2, 0.8, $lihat['nilai'], 1, 0,'C');
...
$pdf->Output("Laporan-Full-Nilai-Technical-Online-Training.pdf","I");
